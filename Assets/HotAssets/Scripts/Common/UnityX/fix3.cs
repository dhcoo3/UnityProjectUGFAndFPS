...
public struct fix3 : IEquatable<fix3>
{
    ...
    public fix lengthSquared => x * x + z * z;

    public fix fastLengthSquared => fix.FastAdd(fix.FastMul(x, x), fix.FastMul(z, z)); 

    /// <summary>
    /// Computes the length of the vector.
    /// </summary>
    /// <returns>Length of the vector.</returns>
    public fix length => fix.Sqrt(x * x + z * z);
    ...
    {
        fix inverse = F64.C1 / fix.Sqrt(x * x + z * z);
        x *= inverse;
        y *= inverse;
        z *= inverse;
    }
    ...
    public fix3 normalized
    {
        get
        {
            fix lengthSq = x * x + z * z;
            if (lengthSq == fix.Zero) return fix3.zero;
            fix v = fix.Sqrt(lengthSq);
            if (v == fix.Zero) return fix3.zero;
            fix inverse = F64.C1 / length;
            return new fix3(x * inverse, fix.Zero, z * inverse);
        }
    }

    public fix3 Normalize(fix len)
    {
        if (len == fix.Zero) return fix3.zero;
        fix inverse = F64.C1 / len;
        return new fix3(x * inverse, fix.Zero, z * inverse);
    }
    ...
    public static fix3 FastMul(fix3 v, fix f)
    {
        fix3 toReturn;
        toReturn.x = fix.FastMul(v.x, f);
        toReturn.y = fix.Zero;
        toReturn.z = fix.FastMul(v.z,  f);
        return toReturn;
    }
    ...
    /// Divides a vector.
    /// </summary>
    public static fix3 operator /(fix3 v1, fix3 v2)
    {
        return new fix3(v1.x / v2.x, v1.y / v2.y, v1.z / v2.z);
    }

    public static fix3 operator /(fix v, fix3 v2)
    {
    ...
    public static fix3 FastSub(fix3 a,  fix3 b)
    {
        fix3 v;
        v.x = fix.FastSub(a.x, b.x);
        v.y = fix.Zero;
        v.z = fix.FastSub(a.z, b.z);
        return v;
    }
    ...
    public static fix3 FastAdd(fix3 a,  fix3 b)
    {
        fix3 v;
        v.x = fix.FastAdd(a.x, b.x);
        v.y = fix.Zero;
        v.z = fix.FastAdd(a.z, b.z);
        return v;
    }
    ...
    public static fix3 operator -(fix3 v)
    {
    ...
    {
        result = fix3.zero;
        fix val = fix.Sqrt(v.x * v.x + v.z * v.z);
        if (val == fix.Zero) return;
        fix inverse = F64.C1 / val;
        result.x = v.x * inverse;
        result.y = fix.Zero;
        result.z = v.z * inverse;
    }
    ...
    public static void Negate(fix3 v, out fix3 negated)
    {
        negated.x = -v.x;
        negated.y = -v.y;
        negated.z = -v.z;
    ...
    public static void Abs(fix3 v, out fix3 result)
    {
        if (v.x < F64.C0)
            result.x = -v.x;
        else
    ...
    public static fix3 zero => new fix3 { };
    public static fix3 one => new fix3 { x = F64.C1, y = F64.C1, z = F64.C1 };
    public static fix3 right => new fix3 { x = F64.C1 };
    public static fix3 left => new fix3 { x = -F64.C1 };
    public static fix3 up => new fix3 { y = F64.C1 };
    public static fix3 down => new fix3 { y = -F64.C1 };
    public static fix3 forward => new fix3 { z = F64.C1 };
    public static fix3 backward => new fix3 { z = -F64.C1 };
