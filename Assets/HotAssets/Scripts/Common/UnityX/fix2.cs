...
public struct fix2 : IEquatable<fix2>
{
    ...
    public fix fastLength => fix.Sqrt(fix.FastMul(x, x) + fix.FastMul(y, y));
    ...
    public static void Divide(fix2 v, fix divisor, out fix2 result)
    {
    ...
    public static void Negate(fix2 v, out fix2 negated)
    {
        negated.x = -v.x;
        negated.y = -v.y;
    }
    ...
    public static void Abs(fix2 v, out fix2 result)
    {
        if (v.x < F64.C0)
            result.x = -v.x;
        else
    ...
    public fix2 normalized
    {
        get
        {
            var t = fix.Sqrt(x * x + y * y);
            if (t == fix.Zero) return fix2.zero;
            fix inverse = F64.C1 / t;
    ...
    /// Returns a normalized version of the vector.
    /// </summary>
    public fix2 fastNormalized
    {
        get
        {
            var t = fix.Sqrt(fix.FastAdd(fix.FastMul(x, x), fix.FastMul(y, y)));
            if (t == fix.Zero) return fix2.zero;
            fix inverse = F64.C1 / t;
            return new fix2(fix.FastMul(x, inverse), fix.FastMul(y, inverse));
        }
    }
    ...
    /// <returns>Divided vector.</returns>
    public static fix2 operator /(fix2 v1, fix2 v2)
    {
        return new fix2(v1.x / v2.x, v1.y / v2.y);
    }

    public static fix2 operator /(fix v, fix2 v2)
    {
    ...
    public static fix2 operator -(fix2 v)
    {
    ...
    /// </summary>
    public static fix2 FastSub(fix2 a, fix2 b) => new fix2(fix.FastSub(a.x, b.x),fix.FastSub(a.y, b.y));
    
    /// <summary>
    /// Subtracts y from x witout performing overflow checking. Should be inlined by the CLR.
    /// </summary>
    public static fix2 FastAdd(fix2 a, fix2 b) => new fix2(fix.FastAdd(a.x, b.x),fix.FastAdd(a.y, b.y));

    public static fix2 FastMul(fix2 a, fix2 b) => new fix2(fix.FastMul(a.x, b.x), fix.FastMul(a.y, b.y));
    
    public static fix2 zero => new fix2(0, 0);
    public static fix2 right => new fix2(1, 0);
    public static fix2 left => new fix2(-1, 0);
    public static fix2 up => new fix2(0, 1);
    public static fix2 down => new fix2(0, -1);

    public static explicit operator fix2(Vector2 v) => new fix2((fix)v.x, (fix)v.y);
    public static explicit operator Vector2(fix2 v) => new Vector2((float)v.x, (float)v.y);
    public static implicit operator fix3(fix2 v) => new fix3(v.x, v.y, 0);
}
