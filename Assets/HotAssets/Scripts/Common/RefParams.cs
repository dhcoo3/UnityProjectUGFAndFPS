...
namespace HotAssets.Scripts.Common
{
    ...
    public class RefParams : IReference
    {
        ...
        {
            GameExtension.VariablePool.SetVariable<T>(Id, key, value);
        }
        ...
        public T Get<T>(string key, T defaultValue = null) where T : Variable
        {
            ...
            return GameExtension.VariablePool.GetVariable<T>(Id, key) ?? defaultValue;
        }

        public bool TryGet<T>(string key, out T value) where T : Variable
        {
            return GameExtension.VariablePool.TryGetVariable<T>(this.Id, key, out value);
        }

        public void Clear()
        {
            ResetProperties();
            GameExtension.VariablePool.ClearVariables(this.Id);
        }
        /// <summary>
