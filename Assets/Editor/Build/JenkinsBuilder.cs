...
namespace Editor.Build
{
    public class JenkinsBuilder
    {
        ...
        public static void BuildResource()
        {
            ...
            var configFile = AssetPathUtil.GetCombinePath(Directory.GetParent(Application.dataPath).FullName, BuildResourceConfigFile);
            ...
            try
            {
                ...
                configJson = JsonUtil.ToObject<JenkinsBuildResourceConfig>(jsonStr);
        ...
        public static void BuildApp()
        {
            ...
            var configFile = AssetPathUtil.GetCombinePath(Directory.GetParent(Application.dataPath).FullName, BuildAppConfigFile);
            if (!File.Exists(configFile))
            {
                Debug.LogError($"构建失败! 构建配置文件不存在:{configFile}");
                return;
            ...
            {
                var jsonStr = File.ReadAllText(configFile);
                configJson = JsonUtil.ToObject<JenkinsBuildAppConfig>(jsonStr);
