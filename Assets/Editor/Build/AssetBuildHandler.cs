...
namespace Editor
{
    public class AssetBuildHandler : IBuildEventHandler
    {
        ...
        public void OnOutputUpdatableVersionListData(Platform platform, string versionListPath, int versionListLength, int versionListHashCode, int versionListCompressedLength, int versionListCompressedHashCode)
        {
            ...
            var outputVersionFile = AssetPathUtil.GetCombinePath(dir, LaunchConst.VersionFile);
            ...
            var outputVersionInfo = new VersionInfo()
            {
                ...
                UpdatePrefixUri = AssetPathUtil.GetCombinePath(AppBuildSettings.Instance.UpdatePrefixUri, platform.ToString()),
            ...
            File.WriteAllText(outputVersionFile, JsonUtil.ToJson(outputVersionInfo));
        }
        ...
        {
            if (!resEditor.HasResource(BuildConst.SharedAssetBundleName, null))
            {
                bool addSuccess = resEditor.AddResource(BuildConst.SharedAssetBundleName, null, null, LoadType.LoadFromMemoryAndQuickDecrypt, false);

                if (!addSuccess)
                {
                    Debug.LogWarningFormat("ResourceEditor Add Resource:{0} Failed!", BuildConst.SharedAssetBundleName);
                    return false;
                }
            }
            ...
            {
                Debug.Log($"-------------添加下列冗余资源到{BuildConst.SharedAssetBundleName}------------");
                var items = resEditor.GetResource(BuildConst.SharedAssetBundleName, null).GetAssets();
                ...
                foreach (var assetName in duplicateAssetNames)
                {
                    ...
                    if (!resEditor.AssignAsset(AssetDatabase.AssetPathToGUID(assetName), BuildConst.SharedAssetBundleName, null))
                    {
                        Debug.LogWarning($"添加资源:{assetName}到{BuildConst.SharedAssetBundleName}失败!");
                    }
                }
                Debug.Log($"-------------处理冗余资源结束------------");
            }
        ...
        private static HashSet<string> FindDuplicateAssetNames(ResourceEditorController resEditor)
        {
            ...
            for (int i = 0; i < resources.Length; i++)
            {
                ...
                if (resource.FullName == BuildConst.SharedAssetBundleName) continue;
