...
namespace Editor.Build
{
    public class HybridCLRExtensionTool
    {
        ...
        public static void CompileTargetDll(bool includeAotDll)
        {
            ...
            var desDir = AssetPathUtil.GetCombinePath(Application.dataPath, LaunchConst.HotFixDLLDir);
            ...
            string content = $"Compile dlls and copy to '{LaunchConst.HotFixDLLDir}' success.";
        ...
        public static string[] CopyHotfixDllTo(BuildTarget target, string desDir, bool copyAotMeta = true)
        {
            ...
            foreach (var dll in HybridCLR.Editor.SettingsUtil.HotUpdateAssemblyNamesIncludePreserved)
            {
                ...
                string dllPath = AssetPathUtil.GetCombinePath(isObfuzDll ? obfuzDllSrcDir : hotfixDllSrcDir, dll + ".dll");
                if (File.Exists(dllPath))
                {
                    string dllBytesPath = AssetPathUtil.GetCombinePath(desDir, Utility.Text.Format("{0}.bytes", dll));
            ...
            var hotfixListFile = AssetPathUtil.GetCombinePath(Application.dataPath, LaunchConst.HotFixDLLDir, "HotfixFileList.txt");
            File.WriteAllText(hotfixListFile, JsonUtil.ToJson(HybridCLR.Editor.SettingsUtil.HotUpdateAssemblyNamesIncludePreserved), System.Text.Encoding.UTF8);
        ...
        public static string[] CopyAotDllsToProject(BuildTarget target)
        {
            ...
            string aotSaveDir = AssetPathUtil.GetCombinePath(Application.dataPath, "Resources", LaunchConst.AotdllDir);
            if (Directory.Exists(aotSaveDir))
            {
                Directory.Delete(aotSaveDir, true);
            }
            Directory.CreateDirectory(aotSaveDir);
            var aotDllEncryptCode = UTF8Encoding.UTF8.GetBytes(LaunchConst.AOTDllsKey);
            foreach (var dll in HybridCLR.Editor.SettingsUtil.AOTAssemblyNames)
            {
                string dllPath = AssetPathUtil.GetCombinePath(aotDllDir, dll + ".dll");
                if (!File.Exists(dllPath))
                {
                ...
                string dllBytesPath = AssetPathUtil.GetCombinePath(aotSaveDir, Utility.Text.Format("{0}.bytes", dll));
        ...
        private static void RefreshAssemblyDefinition(bool disableHybridCLR)
        {
            ...
            var builtinFile = AssetPathUtil.GetCombinePath(assetParentDir, BuildConst.BuiltinAssembly);
            var textData = File.ReadAllText(builtinFile);
            var jsonData = JsonUtil.ToObject<Unity.Plastic.Newtonsoft.Json.Linq.JObject>(textData);
            var refAsmbs = jsonData["references"] as Unity.Plastic.Newtonsoft.Json.Linq.JArray;

            if (refAsmbs.Count() > 0)
            {
                if (refAsmbs[0].Value<string>().StartsWith("GUID:"))
                {
                    EditorUtility.DisplayDialog("Error", Utility.Text.Format("解析Assembly Definition文件{0}失败: 请将其Use GUIDs设置为false后重试!", BuildConst.BuiltinAssembly), "OK");
                    return;
                }
            }
            ...
            if (disableHybridCLR)
            {
                ...
                if (changed)
                {
                    File.WriteAllText(builtinFile, jsonData.ToString(Unity.Plastic.Newtonsoft.Json.Formatting.Indented));
                    AssetDatabase.Refresh();
                }
            }
            else
...
#elif UNITY_MINIGAME
            return UnityEditor.Build.NamedBuildTarget.MiniGame;
#elif UNITY_WEBGL
