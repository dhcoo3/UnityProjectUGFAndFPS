...
public class LoadHotfixDllProcedure : ProcedureBase
{
    ...
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        ...
        Builtin.Scripts.Game.AppEntry.Event.Subscribe(LoadHotfixDllEventArgs.EventId, OnLoadHotfixDllCallback);
#endif
        PreloadAndInitData();
    }
    ...
    {
#if !DISABLE_HYBRIDCLR
        Builtin.Scripts.Game.AppEntry.Event.Unsubscribe(LoadHotfixDllEventArgs.EventId, OnLoadHotfixDllCallback);
#endif
    ...
    {
        //显示进度条
        Builtin.Scripts.Game.AppEntry.BuiltinView.ShowLoadingProgress();
        totalProgress = 0;
        loadedProgress = 0;
...
#if !DISABLE_HYBRIDCLR
        if (!Builtin.Scripts.Game.AppEntry.Base.EditorResourceMode)
        {
    ...
    {
        var aotMetaDlls = Resources.LoadAll<TextAsset>(LaunchConst.AotdllDir);
    ...
    private void LoadMetadata(TextAsset[] aotMetaDlls)
    {
        ...
        var encryptCode = UTF8Encoding.UTF8.GetBytes(LaunchConst.AOTDllsKey);
        foreach (var dll in aotMetaDlls)
        {
            ...
            Log.Info(Utility.Text.Format("补充元数据:{0}. ret:{1}", dll.name, resultCode));
    ...
    {
        Log.Info("Start load hotfix dlls...");
        var hotfixListFile = AssetPathUtil.GetCombinePath("Assets", LaunchConst.HotFixDLLDir, "HotfixFileList.txt");
        if (Builtin.Scripts.Game.AppEntry.Resource.HasAsset(hotfixListFile) == GameFramework.Resource.HasAssetResult.NotExist)
        {
            Log.Fatal("HotfixFileList Not Exist :{0}", hotfixListFile);
            return;
        }
        Builtin.Scripts.Game.AppEntry.Resource.LoadAsset(hotfixListFile, new GameFramework.Resource.LoadAssetCallbacks((string assetName, object asset, float duration, object userData) =>
        {
            var textAsset = asset as TextAsset;
            if (textAsset != null)
            {
                hotfixListIsLoaded = true;
                hotfixDlls = JsonUtil.ToObject<System.Collections.Generic.List<string>>(textAsset.text);
                totalProgress += hotfixDlls.Count;
                if (hotfixDlls.Count == 1)
                {
                    var mainDll = AssetPathUtil.GetHotfixDll(hotfixDlls.Last());
                ...
                else
                {
                    for (int i = 0; i < hotfixDlls.Count - 1; i++)
                    {
                        ...
                        var dllAsset = AssetPathUtil.GetHotfixDll(dllName);
    ...
    private void OnLoadHotfixDllCallback(object sender, GameEventArgs e)
    {
        ...
        {
            Log.Error($"加载dll失败:{args.DllName}");
            return;
        }

        loadedProgress++;
        Builtin.Scripts.Game.AppEntry.BuiltinView.SetLoadingProgress(loadedProgress / (float)totalProgress);
    ...
    {
        Builtin.Scripts.Game.AppEntry.Resource.LoadAsset(dllAssetName, typeof(TextAsset), new LoadAssetCallbacks(OnLoadDllSuccess, OnLoadDllFail), userData);
    }
    ...
    private void OnLoadDllFail(string assetName, LoadResourceStatus status, string errorMessage, object userData)
    {
        ...
        Builtin.Scripts.Game.AppEntry.Event.Fire(this, ReferencePool.Acquire<LoadHotfixDllEventArgs>().Fill(Path.GetFileNameWithoutExtension(assetName), null, userData));
    }
    ...
    private void OnLoadDllSuccess(string assetName, object asset, float duration, object userData)
    {
        ...
        Builtin.Scripts.Game.AppEntry.Event.Fire(this, ReferencePool.Acquire<LoadHotfixDllEventArgs>().Fill(dllName, dllAssembly, userData));
    }
    /// <summary>
